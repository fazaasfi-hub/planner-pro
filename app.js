...
function loadTheme() {
  try {
    ...
    applyCustomTheme();
    const tog = $('#themeToggle');
    if (tog) tog.innerHTML = THEME_ICONS['light'];
  }
}
...
// ============================================================
// EXPORT FUNCTIONS (PENTING!)
// ============================================================
function exportStudySchedule() {
  if (!studyData.length) return toast('Belum ada jadwal', 'info');
  let text = 'JADWAL PELAJARAN\n\n';
  DAYS.slice(0,5).forEach(hari => {
    text += `=== ${hari} ===\n`;
    const data = studyData.filter(s => s.hari === hari);
    if (data.length === 0) text += '  (kosong)\n';
    else data.forEach(s => text += `  ${s.jam} - ${s.mapel} ${s.guru ? '| '+s.guru : ''}\n`);
    text += '\n';
  });
  downloadFile(text, 'jadwal-pelajaran.txt');
}

function exportWorkoutSchedule() {
  if (!workoutData.length) return toast('Belum ada jadwal workout', 'info');
  let text = 'JADWAL WORKOUT\n\n';
  DAYS.forEach(day => {
    const data = workoutData.filter(w => w.day === day);
    if (data.length === 0) return;
    text += `=== ${day} ===\n`;
    data.forEach(w => text += `  ${w.name} - ${w.time} (${w.sets}s x ${w.reps}r @ ${w.kg}kg)\n`);
    text += '\n';
  });
  downloadFile(text, 'jadwal-workout.txt');
}
